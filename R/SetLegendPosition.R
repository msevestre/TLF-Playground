                              ...
                              Subplot.Index = NA)
{
 # Check Plot Handle  
  ...
  if (is.null(Location) || tolower(Location) %in% "none") {
    ...
    if(is.null(X.Location)) {X.Location <- NA}
    if(is.null(Y.Location)) {Y.Location <- NA}
    # Check that Locations are correctly input
    stopifnot(tolower(Location) %in% c("inside", "outside"),
    ...
    if(tolower(Location) %in% "inside"){
      ...
      PlotHandle <- PlotHandle + theme(legend.position = c(X.Location, Y.Location),
    ...
    if(tolower(Location) %in% "outside"){
      ...
      if(!tolower(X.Location) %in% c(NA, "center", "middle") && !tolower(Y.Location) %in% c(NA, "center", "middle")){
        ...
        if (tolower(X.Location) %in% "left") {X.Location <- 0}
        if (tolower(X.Location) %in% "right") {X.Location <- 1}
        PlotHandle <- PlotHandle + theme(legend.position = tolower(Y.Location),
